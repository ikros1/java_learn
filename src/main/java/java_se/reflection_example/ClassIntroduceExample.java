...
public class ClassIntroduceExample {
    /*
...
因为反射的目的是为了获得某个实例的信息。因此，当我们拿到某个Object实例时，我们可以通过反射获取该Object的class信息：
    ...
    static void printClassInfo(Class cls) throws InstantiationException, IllegalAccessException {
        ...
        // 创建一个String实例:
        String s = (String) clss.newInstance();
        //上述代码相当于new String()。通过Class.newInstance()可以创建类实例，它的局限是：
        // 只能调用public的无参数构造方法。
        // 带参数的构造方法，或者非public的构造方法都无法通过Class.newInstance()被调用。
    }
    /*
    注意到数组（例如String[]）也是一种类，而且不同于String.class，
    它的类名是[Ljava.lang.String;。此外，JVM为每一种基本类型如int也创建了Class实例，
    通过int.class访问。
...
这就是为什么我们只需要把Log4j的jar包放到classpath中，
