...
public class ReaderExample {
    public static void main(String[] args) throws IOException {
        Reader reader = new FileReader("src/main/resources/log4j.properties"); // 字符编码是???
        for (;;) {
            int n = reader.read(); // 反复调用read()方法，直到返回-1
            if (n == -1) {
                break;
        ...
        reader.close(); // 关闭流
        //更优的办法
        try (Reader reade = new FileReader("src/main/resources/log4j.properties", StandardCharsets.UTF_8)) {
            // TODO
            char[] buffer = new char[1000];
            int n;
            while ((n = reade.read(buffer)) != -1) {
                System.out.println("read " + n + " chars.");
