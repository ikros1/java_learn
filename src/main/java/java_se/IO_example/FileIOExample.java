...
public class FileIOExample {
    ...
    public static void main(String[] args) throws IOException {
        ...
        File f1 = new File("C:"+File.separator+"Windows");
        File f2 = new File("C:"+File.separator+"Windows"+File.separator+"notepad.exe");
        File f3 = new File("C:"+File.separator+"Windows"+File.separator+"nothing");
        System.out.println(f1.isFile());
        System.out.println(f1.isDirectory());
        System.out.println(f2.isFile());
        System.out.println(f2.isDirectory());
        ...
        File f_temp = File.createTempFile("tmp-", ".txt"); // 提供临时文件的前缀和后缀
        f_temp.deleteOnExit(); // JVM退出时自动删除
        System.out.println(f_temp.isFile());
        System.out.println(f_temp.getAbsolutePath());


        File f_s = new File("C:"+File.separator+"Windows");
        File[] fs1 = f_s.listFiles(); // 列出所有文件和子目录
        printFiles(fs1);
        File[] fs2 = f_s.listFiles(new FilenameFilter() { // 仅列出.exe文件
            public boolean accept(File dir, String name) {
                return name.endsWith(".exe"); // 返回true表示接受该文件
            }
        });
        ...
        //Path p1 = Paths.get(".", "project", "study"); // 构造一个Path对象
        Path p1 = Paths.get("C:", "Windows", "Temp");
        System.out.println(p1);
        Path p2 = p1.toAbsolutePath(); // 转换为绝对路径
        System.out.println(p2);
        Path p3 = p2.normalize(); // 转换为规范路径
        System.out.println(p3);
        File fi = p3.toFile(); // 转换为File对象
        System.out.println(fi);
        // 获取Temp目录的绝对路径
        Path tempDir = Paths.get("C:", "Windows", "Temp").toAbsolutePath();
